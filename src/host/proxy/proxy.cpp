...
int nvshmemi_proxy_setup_device_channels(proxy_state_t *state) {
    ...
    /* The requirement below is because of the way g_coalescing is implemented */
    assert(static_cast<uint64_t>(G_COALESCING_BUF_SIZE) >= (proxy_channel_g_buf_size * 16));
...
inline int process_channel_dma(proxy_state_t *state, proxy_channel_t *ch, int *is_processed,
    ...
    // issue transport DMA
    {
        ...
        if (tcurr->host_ops.rma_batch_hint) {
            ...
            tcurr->host_ops.rma_batch_hint(tcurr, NVSHMEMX_QP_DEFAULT, hint_flags);
        }
