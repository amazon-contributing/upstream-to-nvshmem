...
static int nvshmemt_libfabric_gdr_process_completion(nvshmem_transport_t transport,
                                                     ...
                                                     struct fi_cq_data_entry *entry,
                                                     fi_addr_t *addr);
static int nvshmemt_libfabric_process_completions(nvshmem_transport_t transport, int qp_index);
...
static inline int try_again(nvshmem_transport_t transport, int *status, uint64_t *num_retries,
                            int qp_index, nvshmemt_libfabric_try_again_call_site_t call_site,
                            bool completions_only = false);
int gdrcopy_amo_ack(nvshmem_transport_t transport, nvshmemt_libfabric_endpoint_t &ep,
                    fi_addr_t dest_addr, uint16_t ack_seq_num,
                    uint8_t ack_count, uint8_t ack_num_ops);
...
nvshmemt_libfabric_imm_cq_data_hdr_t get_write_with_imm_hdr(uint64_t imm_data) {
    return (nvshmemt_libfabric_imm_cq_data_hdr_t)((uint32_t)imm_data >>
                                                  NVSHMEM_STAGED_AMO_IMM_HEADER_BIT_SHIFT);
}
...
static inline int try_again(nvshmem_transport_t transport, int *status, uint64_t *num_retries,
              int qp_index,
              nvshmemt_libfabric_try_again_call_site_t call_site, bool completions_only) {
    if (likely(*status == 0)) {
        return 0;
    }
...
int gdrcopy_amo_ack(nvshmem_transport_t transport, nvshmemt_libfabric_endpoint_t &ep,
    ...
    do {
        status = libfabric_state->op_queue[ep.domain_index]->getNextSends(
            (void **)(&send_elem), 1);
    } while (try_again(transport, &status, &num_retries, ep.qp_index,
                       NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_GDRCOPY_AMO_ACK, true));
    if (status) goto out;

    ack_op = (nvshmemt_libfabric_gdr_amo_ack_op_t *)send_elem;
    ...
    ep.submitted_ops++;
    NVSHMEM_TRACE_RECEIVER_POST_ACK(convert_addr_to_pe(libfabric_state, &ep, dest_addr), ep.domain_index, ack_seq_num, ack_op->type);
...
int perform_gdrcopy_amo(nvshmem_transport_t transport, nvshmemt_libfabric_gdr_op_ctx_t *op,
    ...
    volatile T *ptr;
    int status = 0;
    NVSHMEM_TRACE_RECEIVER_GDRCOPY_START(op->ep_index, received_op->op, op->type == NVSHMEMT_LIBFABRIC_MATCH ? 1 : 0);
...
static int nvshmemt_libfabric_gdr_complete_amos(nvshmem_transport_t transport) {
    nvshmemt_libfabric_state_t *libfabric_state = (nvshmemt_libfabric_state_t *)transport->state;
    signal_delivery_done_entry done;
    nvshmemt_libfabric_signal_state_t *signal_state = nullptr;
    uint64_t num_retries = 0;
    int send_elems_index = 0;
    int status = 0;
    ...
    status = fi_recv(done.ep->endpoint, (void *)done.op, NVSHMEM_STAGED_AMO_WIREDATA_SIZE,
                     fi_mr_desc(libfabric_state->mrs[done.ep->domain_index]),
    ...
    if (done.is_fetch_amo) {
        ...
        } while (try_again(transport, &status, &num_retries, done.ep->qp_index,
                           NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_PERFORM_GDRCOPY_AMO_SEND,
                           true));
    ...
    signal_state = (done.ep->domain_index == 0) ? &libfabric_state->host_signal_state
...
out:
    NVSHMEM_TRACE_RECEIVER_GDRCOPY_END(done.op->ep_index, status);
    return status;
}
...
int nvshmemt_libfabric_ack_aggregator::flush_peer(
    ...
    status = gdrcopy_amo_ack(transport, *ep, dest_addr,
                             pending.range_end,
                             (uint8_t)pending.range_count,
                             (uint8_t)(pending.signal_ack_count + pending.amo_ack_count));
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
bool nvshmemt_libfabric_ack_aggregator::try_extract_for_peer(
    int pe, uint16_t &range_end, uint16_t &range_count, uint16_t &signal_ack_count) {
    auto &pending = pending_per_peer[pe];
    if (pending.total_pending() == 0)
        return false;
...
static void nvshmemt_libfabric_apply_ack(nvshmemt_libfabric_signal_state_t *signal_state,
                                         int pe,
                                         const nvshmemt_libfabric_ack_payload_t &ack) {
    if (ack.ack_count > 0) {
        uint16_t end_seq = nvshmemt_libfabric_endpoint_seq_counter_t::seq_num_wrapup(ack.ack_seq_num);
        signal_state->put_signal_seq_counter_per_pe[pe]
            .return_acked_range(end_seq, ack.ack_count);
    }
...
static int nvshmemt_libfabric_gdr_process_completion(nvshmem_transport_t transport,
    ...
    if (entry->flags & FI_REMOTE_CQ_DATA) {
        ...
        if (NVSHMEMT_LIBFABRIC_IMM_PUT_SIGNAL_SEQ == imm_header ||
            ...
            NVSHMEM_TRACE_WRITE_REMOTE_COMPLETION(ep.domain_index, (uint64_t)(addr ? *addr : 0), (int)imm_header);
            goto out;
        } else {
    ...
    if (entry->flags & FI_SEND) {
        NVSHMEM_TRACE_SEND_COMPLETION(ep.domain_index, (uint64_t)entry->op_context, op->type);
        state->op_queue[domain_idx]->putToSend(op);
        ep.completed_ops++;
    } else if (entry->flags & FI_RMA) {
        NVSHMEM_TRACE_WRITE_COMPLETION(ep.domain_index, (uint64_t)entry->op_context, op->type);
        ...
        status = nvshmemt_libfabric_put_signal_completion(transport, ep, entry, addr);
        NVSHMEM_TRACE_RECV_COMPLETION(ep.domain_index, addr ? (uint64_t)*addr : 0, op->type);
    } else if ((entry->flags & FI_RECV) && (op->type == NVSHMEMT_LIBFABRIC_AMO_ACK_SEND)) {
        nvshmemt_libfabric_gdr_amo_ack_op_t *ack_op =
                (nvshmemt_libfabric_gdr_amo_ack_op_t *)op;
        nvshmemt_libfabric_put_signal_ack_completion(state, ep, ack_op, *addr);
        NVSHMEM_TRACE_RECV_COMPLETION(ep.domain_index, (uint64_t)entry->op_context, op->type);
...
static int nvshmemt_libfabric_put_signal_completion(nvshmem_transport_t transport,
    ...
    if (is_write_comp) {
        ...
        map_seq = (uint32_t)entry->data & NVSHMEM_STAGED_AMO_PUT_SIGNAL_SEQ_CNTR_BIT_MASK;
        progress_count = -1;
    } else {
    ...
    if (is_write_comp && get_write_with_imm_hdr(entry->data) == NVSHMEMT_LIBFABRIC_IMM_STANDALONE_PUT_WITH_ACK_REQ) {
        ...
        ack_comp_entry.ack_entry.put_count = ((uint32_t)entry->data >> NVSHMEM_STAGED_AMO_PUT_SIGNAL_SEQ_CNTR_BIT_SHIFT) & 0xFF;
        signal_state->proxy_put_signal_comp_map[pe].insert(map_seq, ack_comp_entry);
    } else {
        auto *slot = signal_state->proxy_put_signal_comp_map[pe].find(map_seq);
        if (slot) {
            ...
            signal_state->proxy_put_signal_comp_map[pe].insert(map_seq, sig_comp_entry);
            slot = signal_state->proxy_put_signal_comp_map[pe].find(map_seq);
        }
...
static int nvshmemt_libfabric_quiet(struct nvshmem_transport *tcurr, int pe, int qp_index) {
    ...
    for (;;) {
        ...
        if (signal_state->ack_aggregator) {
            ...
            status = signal_state->ack_aggregator->flush_all(tcurr, libfabric_state->eps[ep_idx].get());
            if (unlikely(status)) break;
        }
...
static int nvshmemt_libfabric_rma_impl(struct nvshmem_transport *tcurr, int pe, rma_verb_t verb,
    ...
    if (likely(verb.desc == NVSHMEMI_OP_PUT)) {
        ...
        if (libfabric_state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
            ...
            assert(imm_data); // EFA provider requires immediate data for p/put
            do {
                status = fi_writedata(ep.endpoint, &p_buf->p_op.value, op_size,
                                  fi_mr_desc(libfabric_state->mrs[domain_idx]), *imm_data, target_ep,
...
static int nvshmemt_libfabric_rma(struct nvshmem_transport *tcurr, int pe, rma_verb_t verb,
    ...
    if (use_staged_atomics &&
        ...
        imm_data_val = ((uint32_t)header << NVSHMEM_STAGED_AMO_IMM_HEADER_BIT_SHIFT) |
                       ((uint32_t)put_ack_count << NVSHMEM_STAGED_AMO_PUT_SIGNAL_SEQ_CNTR_BIT_SHIFT) |
                       (sequence_count & NVSHMEM_STAGED_AMO_PUT_SIGNAL_SEQ_CNTR_BIT_MASK);
...
static int nvshmemt_libfabric_amo(struct nvshmem_transport *transport, int pe, void *curetptr,
    ...
    NVSHMEM_TRACE_SENDER_POST_AMO(pe, ep.domain_index, verb.desc);
...
static int nvshmemt_libfabric_gdr_signal(struct nvshmem_transport *transport, int pe,
    ...
    /* Piggyback pending ACK for this destination PE if available */
    {
        ...
        if (signal_state->ack_aggregator &&
            ...
            signal->ack.ack_count = (uint8_t)ack_range_count;
            assert(ack_signal_count <= UINT8_MAX);
            signal->ack.ack_num_ops = (uint8_t)ack_signal_count;
        } else {
...
skip:
    ...
    for (int ep_idx = ep_idx_start; ep_idx < ep_idx_end; ep_idx++) {
        ...
        } while (try_again(tcurr, &status, &num_retries, 0,
                           NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_ENFORCE_CST));
...
static void nvshmemt_libfabric_cleanup_signal_ordering_state(nvshmemt_libfabric_state_t *state)
{
    ...
    state->host_signal_state.ack_aggregator = nullptr;
    state->host_signal_state.put_signal_seq_counter_per_pe.clear();
    ...
    state->proxy_signal_state.ack_aggregator = nullptr;
...
static int nvshmemt_libfabric_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    /* Initialize state-level signal ordering state */
    {
        ...
        uint32_t hwm = (min_tx_size > 256) ? min_tx_size - 256 : min_tx_size / 2;
...
static int nvshmemt_libfabric_finalize(nvshmem_transport_t transport) {
    ...
    libfabric_state->signal_work_queue.~SPSCRing();
    libfabric_state->signal_done_queue.~SPSCRing();
